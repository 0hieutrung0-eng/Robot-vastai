...
print(f"[START] Robot sử dụng API v1 - Max {MAX_INSTANCES} máy")

def get_running_count():
    try:
        r = requests.get(f"{BASE_URL}/instances/", headers=HEADERS, timeout=20)
        print(f"[API v1] Status: {r.status_code}")
...
while True:
    ...
    try:
        resp = requests.post(f"{BASE_URL.replace('/v1', '/v0')}/bundles/", headers=HEADERS, json=search_payload, timeout=15)
...
"""
            ...
            rent_resp = requests.put(f"{BASE_URL.replace('/v1', '/v0')}/asks/{offer_id}/", headers=HEADERS, json=rent_payload, timeout=35)

...
while True:
    ...
    for inst in instances:
        ...
        status_msg = str(inst.get("status_msg", "")).lower() # Quét thêm thông báo lỗi hệ thống
        gpu = inst.get("gpu_name", "Unknown")
        
        # Bổ sung từ khóa để xóa triệt để nếu thuê phải máy lỗi OCI/Runc từ nhà cung cấp
        if any(err in status or err in actual_status or err in status_msg for err in ["error", "failed", "storage", "oci", "daemon", "runc"]):
    ...
    try:
        ...
        if resp.status_code == 200 and resp.json().get("offers"):
            ...
            # ĐÃ SỬA: Tạo môi trường ảo python3-venv độc lập và chặn container tự sập bằng lệnh "sleep infinity"
            onstart_cmd = (
            ...
            if rent_resp.status_code in (200, 201):
                print(f"[🎉] GỬI LỆNH THUÊ THÀNH CÔNG {gpu}!")
                time.sleep(180)   # Giảm thời gian chờ xuống 3 phút để bot nhanh chóng quay lại quản lý và dọn máy nếu dính lỗi
            else:
